...
import {AlignCenterIcon, AlignJustifyIcon, AlignLeftIcon, AlignRightIcon, BoldIcon, HtmlIcon, IndentDecreaseIcon, IndentIncreaseIcon, ItalicIcon, LinkIcon, LinkOffIcon, ListIcon, ListNumberIcon, MicrophoneIcon, MovieIcon, PhotoIcon, SourceCodeIcon, StyleClearIcon} from "../assets/icons/Icons.jsx";
...
const Editor = ({
    ...
    const editor = useEditor({
        extensions: [
            ...
            Link.configure({
                openOnClick: false,
                HTMLAttributes: {
                    class: 'text-blue-500 hover:text-blue-700', // استایل‌های دلخواه
                },
            }),
            OrderedList.configure({
                HTMLAttributes: {
                    class: 'list-none pr-0 rtl:pl-0 counter-reset: item', // استفاده از list-none برای کنترل دستی marker
                },
            }),
            BulletList.configure({
                HTMLAttributes: {
                    class: 'list-none pr-0 rtl:pl-0', // استفاده از list-none برای کنترل دستی marker
                },
            }),
            ListItem.configure({
                HTMLAttributes: {
                    class: 'relative pl-6 rtl:pr-6 rtl:pl-0 my-1', // استفاده از relative برای pseudo-element
                },
            }),
            Image,
            Video,
    ...
    // eslint-disable-next-line react-hooks/rules-of-hooks
    const [openUploadImage, setOpenUploadImage] = useState(false); // ذخیره فونت انتخابی
    // eslint-disable-next-line react-hooks/rules-of-hooks
    const [openUploadVideo, setOpenUploadVideo] = useState(false); // ذخیره فونت انتخابی
    // eslint-disable-next-line react-hooks/rules-of-hooks
    const [openUploadAudio, setOpenUploadAudio] = useState(false); // ذخیره فونت انتخابی
    ...
    return (
        <div className="relative nilfam-editor flex flex-col p-0.5 gap-0.5 border border-gray-200 rounded-xl " dir={Configs.RtlLang.includes(lang) ? "rtl" : "ltr"}>
            ...
            <div className="add-font flex flex-col sticky top-0 z-10 bg-white p-1 border-b border-gray-200">
                ...
                <div className="flex flex-wrap items-center gap-1">
                    ...
                    <button className="class-button data-active:bg-gray-300" data-active={editor.isActive('bold') || null} onClick={() => editor.chain().focus().toggleBold().run()} title={t('bold', lang)}><BoldIcon/></button>
                    <button className="class-button data-active:bg-gray-300" data-active={editor.isActive('italic') || null} onClick={() => editor.chain().focus().toggleItalic().run()} title={t('italic', lang)}><ItalicIcon/></button>
                    ...
                    <button className="class-button" onClick={() => editor.chain().focus().unsetLink().run()} title={t('unsetLink', lang)}><LinkOffIcon/></button>
                    <button className="class-button" onClick={() => {editor.chain().focus().unsetTextStyle().clearNodes().run();}} title={t('clearStyle', lang)}><StyleClearIcon/></button>
                    ...
                    <button className="class-button" onClick={() => editor.chain().focus().toggleBulletList().run()} title={t('list', lang)}><ListIcon/></button>
                    <button className="class-button" onClick={() => editor.chain().focus().toggleOrderedList().run()} title={t('listNumber', lang)}><ListNumberIcon/></button>
                    <button className="class-button" onClick={() => {editor.chain().focus().setTextAlign("right").run();}} title={t('alignRight', lang)}><AlignRightIcon/></button>
                    <button className="class-button" onClick={() => {editor.chain().focus().setTextAlign("center").run();}} title={t('alignCenter', lang)}><AlignCenterIcon/></button>
                    <button className="class-button" onClick={() => {editor.chain().focus().setTextAlign("left").run();}} title={t('alignLeft', lang)}><AlignLeftIcon/></button>
                    <button className="class-button" onClick={() => {editor.chain().focus().setTextAlign("justify").run();}} title={t('justify', lang)}><AlignJustifyIcon/></button>
                    <button className="class-button" onClick={bringForward} title={t('forward', lang)}><IndentDecreaseIcon/></button>
                    <button className="class-button" onClick={sendBackward} title={t('backward', lang)}><IndentIncreaseIcon/></button>
                    ...
                    <button className="class-button" onClick={toggleHTML}>
                        {showHTML ? <div><SourceCodeIcon/></div> : <div><HtmlIcon/></div>}
                    </button>
            ...
            {showHTML ? (
                <textarea className="bg-gray-200 p-2 min-h-[300px]" rows={10} value={htmlCode} onChange={(e) => setHtmlCode(e.target.value)}/>
            ) : (
                <EditorContent onClick={() => !editor.isFocused && editor.chain().focus().run()} editor={editor} data-active={editor.isFocused || null} className={`${Configs.RtlLang.includes(lang) ? "text-right" : "text-left"} p-2 data-active:ring-2 data-active:ring-red-200 min-h-[300px] bg-white !outline-none`}/>
            )}

            <div className="add-font text-gray-400 text-sm px-2 border-t border-gray-200">NilfamEditor.ir</div>
