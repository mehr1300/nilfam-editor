...
const BorderColor = ({editor, lang}) => {
    ...
    return (
        <div className="tw:relative" ref={containerRef}>
            <button className="class-button" title={t('borderColor', lang)} onClick={toggleDropdown}>
                <BorderIcon/>
            </button>
                ...
                <div className={`${Configs.RtlLang.includes(lang) ? "tw:right-0" : "tw:left-0"} tw:absolute tw:top-8  tw:z-10`}>
                    <div className="tw:p-2 tw:bg-gray-200 tw:dark:bg-gray-800 tw:w-52 tw:flex tw:flex-col tw:rounded">
                        ...
                        <div className="tw:grid tw:grid-cols-7 tw:gap-2 tw:pe-3 tw:h-52 tw:overflow-y-auto">
                            {Colors.map((color) => (
                                <span key={color}
                                      className="tw:border tw:border-gray-300 tw:dark:border-gray-700 tw:cursor-pointer tw:w-5 tw:h-5 tw:rounded tw:hover:opacity-70" style={{backgroundColor: color}} onClick={() => {
